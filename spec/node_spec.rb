...
describe Node do
  ...
  let(:next_node) {  Node.new('next_node', nil) }
  let(:prev_node) {  Node.new('prev_node', nil) }

  it 'should accept a string as a value and nil as a value for prev_node & next_node' do
    expect {Node.new('string',nil,nil)}.to_not raise_error
  end

  it 'should not accept a string as next_node' do
    expect {Node.new('string',nil,'string')}
    .to raise_error Node::InvalidNextNode, 'Next node must be a Node or be nil.'
  end

  it 'should accept a Node as a value for next_node' do
    expect {Node.new('string',nil, next_node)}.to_not raise_error
  end

  it 'should accept a Node as a value for prev_node' do
    expect {Node.new('string',prev_node)}.to_not raise_error
  end

  it 'should not accept a string as a value for prev_node' do
    expect {Node.new('string','string')}
    .to raise_error Node::InvalidPrevNode, 'Prev node must be a Node or be nil.'
  end

  describe 'other nodes' do

    before(:each) do
      @first_node = Node.new('prev_node')
      @second_node = Node.new('next_node')
      third_node = Node.new('string',@first_node,@second_node)
  ...
  describe 'returning the correct values for next and prev nodes' do
    let(:node) { Node.new('string',prev_node,next_node)}
